...
**It empties the host; it does not keep it empty.** `min_idle` is a floor the daemon
maintains, so a pool that keeps one warm has it back on the next tick — the command says how
many are coming back rather than letting you run it twice and conclude it did not work.

To keep the host quiet, pick the one that matches what you mean:
