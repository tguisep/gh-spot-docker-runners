---
title: Runner images
description: The variants, what is in them, and building them on this host.
slug: 0.6/guides/host/images
---
...
* The apt toolset GitHub installs on its own hosted runners, plus `git`, `cmake`, `node`/`npm`
  and a working `pip`.
* **No language toolchains.** `actions/setup-python`, `setup-go`, `setup-java` and the rest
  fetch what they need at runtime — workflows work as-is, just slower on a cold runner than on
  GitHub's toolcache-equipped images. See [`images/runner/README.md`](https://github.com/tguisep/gh-spot-docker-runners/blob/main/images/runner/README.md) for
...
`build.sh` detects the host's `docker` group and builds it in.

* Mismatch → jobs fail with `permission denied` on `/var/run/docker.sock`, which looks nothing
  like an image problem.
* The images assert it at build time: on the RHEL family the Docker CE package creates its own
  `docker` group first, and a plain `groupadd` silently does nothing.
