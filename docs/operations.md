...
carries **every** label the job asks for, so the pool's `labels` must be a superset:
...
Change both together, or jobs queue forever with no runner to take them.
